...
const RorschachTest = () => {
    ...
    return (
        <Layout
            ...
            description="Faites passer un test de Rorschach en ligne."
        >
            <div className="flex flex-col items-center justify-center mb-12">
                {planchesArray.map(plancheNumber => {
                    return resultsArray.map(result => {
                        if (
                            selectedPlanche === plancheNumber &&
                            resultNumber === result
                        )
                            return (
                                <Planche
                                    key={plancheNumber.toString() + result}
                                    plancheNumber={plancheNumber.toString()}
                                    savePlanche={savePlanche}
                                    savedPlanches={savedPlanches}
                                    resultNumber={result}
                                    updateResultNumber={updateResultNumber}
                                />
                            );
                    });
                })}
                ...
                <div className="flex justify-around items-center w-full">
                    <button
                        className="border shadow-sm rounded-lg p-3 hover:bg-gray-100 text-center"
                        onClick={previousPlanche}
                    >
                        Retour
                    </button>
                        ...
                        {styleArray.map(plancheNumber => {
                            return (
                                <div
                                    key={plancheNumber}
                                    onClick={() =>
                                        switchSelectedPlanche(plancheNumber)
                                    }
                                    className={`w-1 h-1 md:w-2 md:h-2 lg:w-3 lg:h-3 rounded-full cursor-pointer ${isActive[plancheNumber]}`}
                                ></div>
                            );
                        })}
                    </div>
